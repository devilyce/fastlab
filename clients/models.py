...
class Client(models.Model):
    ...
    reference_number = models.ForeignKey(Customers, on_delete=models.SET_NULL, null=True, blank=True)
    ...
    pending_test = models.CharField(max_length=120, choices=STATUS, default='PENDING', null=True, blank=True)
    pending_result = models.CharField(max_length=120, choices=STATUS, default='PENDING', null=True, blank=True)
    result_sent = models.CharField(max_length=120, choices=STATUS, default='PENDING', null=True, blank=True)
    ...
    def save(self, *args, **kwargs):
        if self.client_id is None:
            self.client_id = str(uuid.uuid4())[:11].replace('-', '').lower()
        return super().save(*args, **kwargs)

    def get_absolute_url(self):
        return reverse('admin_view_client', kwargs={'pk': self.pk})


class lab_Client(models.Model):
